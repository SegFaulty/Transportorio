			...
			if self.filter.group_by_city and #city_trades > 0 then
				...
				if city_name_map == "" then
					city_header.add{
						...
						tags={position = city.center, text = city_name .. " #" .. city_index},
...
--			city_name = city_name .. " " .. orientation
		...
		if dominant_item ~= nil then
			...
			dominant_item_translations["explosive"] = "boom"
		...
		local rocks_count = game.surfaces[1].count_entities_filtered{ area=city_environment_area, name={"rock-medium","rock-tiny","rock-small","sand-rock-medium","sand-rock-small","cliff"} }
		...
		elseif trees_count > 100 then
			...
			city_environment_name = parts[1 + city_index % #parts] -- pseudo random
		elseif rocks_count > 70 then
				local parts = { "mountain", "castle", "cliff" }
				city_environment_name = parts[1 + city_index % #parts] -- pseudo random
		elseif water_count > 3 then
			local parts = { "lake", "swamp" }
			city_environment_name = parts[1 + city_index % #parts] -- pseudo random
		elseif rocks_count > 10 then
			local parts = { "boulder", "hill", "val" }
			city_environment_name = parts[1 + city_index % #parts] -- pseudo random
...
function Trades_menu:get_dominant_item(city, min_count)
	local item_counts = {}
	local max_count = 0
